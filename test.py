...
for i in range(len(num_measurements)):
    ...
    x_hat_dip = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e2, output_size=wave_len, num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip2 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e2, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip3 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e2, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    ...
    print("\nWD 1e2 - " + str(num_measurements[i]) + " :", mseone)

    x_hat_dip = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e1, output_size=wave_len, num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip2 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e1, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip3 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e1, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    ...
    print("WD 1e1 - " + str(num_measurements[i]) + " :", msetwo)

    x_hat_dip = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=0, output_size=wave_len, num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip2 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=0, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip3 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=0, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    ...
    print("WD 0 - " + str(num_measurements[i]) + " :", msethree)

    x_hat_dip = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=5e-1, output_size=wave_len, num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip2 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=5e-1, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip3 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=5e-1, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    ...
    print("WD 5e-1 - " + str(num_measurements[i]) + " :", msefour)

    x_hat_dip = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e-1, output_size=wave_len, num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip2 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e-1, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    x_hat_dip3 = dip_utils.run_DIP(phi, y, dtype, LR=LR, MOM=MOM, WD=1e-1, output_size=wave_len,
                                  num_measurements=num_measurements[i], CUDA=CUDA, num_iter=NUM_ITER, alpha_tv=alpha_tv)
    ...
    print("WD 1e-1 - " + str(num_measurements[i]) + " :", msefive)
...
plt.figure()
plt.plot(num_measurements, mse_list[:, 0], label = "10", color = 'r', marker = 'o')
plt.plot(num_measurements, mse_list[:, 1], label = "1", color = 'b', marker = 'D')
plt.plot(num_measurements, mse_list[:, 2], label = "0", color = 'g', marker = '+')
plt.plot(num_measurements, mse_list[:, 3], label = "0.5", color = 'k', marker = '^')
plt.plot(num_measurements, mse_list[:, 4], label = "0.1", color = 'y', marker = 's')
plt.xlabel("Num Measurements")
plt.ylabel("MSE")
plt.title(filename + "-" + test_type + noise_str  + " WD parameter comparison")
plt.legend()
plt.savefig(save_loc + filename + "-" + test_type + noise_str + "-" + str(NUM_ITER) + "iter_WD_comp.jpg")
plt.show()
