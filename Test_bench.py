...
test_type = "Imputation" #Imputation, CS, or Denoising
data_type = "Audio" #Audio, Artificial, or Natural
sample = "captain" #name of the sample
...
if data_type == "Audio":
    ...
    tau = 30
    seed = 1972
    synth = inverse_utils.get_artificial(sample_len=LENGTH, tau=30, seed=1972, n_samples=1)
    x0 = np.zeros((LENGTH, 1))
    x0[:,0] = np.squeeze(synth)

    x = inverse_utils.normalise(x0)

    inverse_utils.save_data(x, save_loc + "Artificial" + "-" + tau + "-" + seed) #save the new wave
...
for i in range(len(num_measurements)):
    ...
    inverse_utils.save_matrix(phi, save_loc + test_type + "/" + sample + "-" + str(num_measurements[i]))
    ...
    for t in range(num_instances):
        ...
        if test_type == "Imputation": #for imputation, we want to add back in the values we know to the predictions
            removed_samples = [x for x in range(0, LENGTH) if x not in kept_samples]
